...
const Notes = () => {
  ...
  const handleUpdateNote = () => {
    if (noteTitle.trim() && noteContent.trim()) {
      const updatedNotes = subjects[currentSubject].map((note) =>
        note.id === editNoteId ? { ...note, title: noteTitle, content: noteContent } : note
      );
      setSubjects({
        ...subjects,
  ...
  const handleDeleteNote = (noteId) => {
    const updatedNotes = subjects[currentSubject].filter((note) => note.id !== noteId);
    setSubjects({
      ...subjects,
      [currentSubject]: updatedNotes,
    });
      ...
      <Modal visible={isModalVisible} animationType="slide" transparent>
        <View style={styles.modalContainer}>
          <View style={styles.modalContent}>
            <Text style={styles.modalTitle}>{editNoteId ? 'Edit Note' : 'Add Note'}</Text>
            <TextInput
            ...
            >
              <Text style={styles.modalButtonText}>{editNoteId ? 'Update' : 'Add'}</Text>
            </TouchableOpacity>
            <TouchableOpacity
              style={styles.modalButton}
